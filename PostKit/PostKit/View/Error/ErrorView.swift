...
struct ErrorView: View {
    ...
    var body: some View {
            ...
            AlertCustomDoubleBtn(
                ...
                topAction: {
                    if errorReasonState == .apiError {
                        Mixpanel.mainInstance().track(event: "Regenerate", properties: ["State": "API Error"])
                    }
        ...
        .onAppear  {
            if errorReasonState == .apiError {
                Mixpanel.mainInstance().track(event: "Generation Failed", properties: ["Error Type": "API Error"])
            }
            else if errorReasonState == .networkError {
                Mixpanel.mainInstance().track(event: "Generation Failed", properties: ["Error Type": "Network Error"])
            }
        }
    }
}
